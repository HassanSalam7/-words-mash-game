...
export interface UnifiedGameMode {
  id: 'story-writing' | 'translation-multiple-choice' | 'translation-typing' | 'translation-metaphorical' | 'speech-training'
  name: string
  icon: string
  gameType: 'story-writing' | 'translation' | 'speech-training'
  ...
  return (
    <div className="w-full px-2 sm:px-4">
      <div className="text-center mb-6 sm:mb-8">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">Choose Your Game Mode</h2>
        <p className="text-sm sm:text-base text-gray-600">Select a game mode to start playing</p>
      </div>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-6 max-w-6xl mx-auto">
        {unifiedGameModes.map((mode) => (
          <Card
            key={mode.id}
            className={`cursor-pointer transition-all duration-300 hover:shadow-xl hover:scale-[1.02] sm:hover:scale-105 group game-card min-h-[120px] sm:min-h-[140px] ${
          ...
          >
            <div className="p-3 sm:p-4 md:p-6 text-center h-full flex flex-col justify-between min-h-[120px] sm:min-h-[140px]">
              <div className="flex-1 flex flex-col justify-center">
                <div className="text-2xl sm:text-3xl md:text-4xl mb-2 sm:mb-3 group-hover:scale-110 transition-transform duration-200 drop-shadow-sm">
                  {mode.icon}
                </div>
                <h3 className="font-bold text-gray-800 text-xs sm:text-sm md:text-base mb-1 sm:mb-2 text-shadow leading-tight">
                  {mode.name}
                </h3>
              </div>
              {mode.difficulty && (
